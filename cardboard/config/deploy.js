...
module.exports = function(deployTarget) {
  ...
  if (deployTarget === 'staging') {
    ...
    ENV.s3Assets.bucket = ENV.s3Pages.bucket = 'staging-cardboard-web';
  ...
  if (deployTarget === 'sandbox') {
    ENV.build.environment = 'production';
    ENV.s3Assets.bucket = ENV.s3Pages.bucket = 'sandbox-cardboard-web';
    ENV.s3Assets.region = ENV.s3Pages.region = 'us-east-1';
  ...
  ENV.slack = {
    ...
    didActivate: function() {
      return function(slack) {
        return slack.notify({
          attachments: [{
            fallback: "Successfully deployed `cardboard` commit " + `${sha} to ${targetName} (${deployTarget})`,
            text: "Successfully deployed `cardboard` commit " + `${sha} to ${targetName} (${deployTarget})`,
            color: "good"
          }]
        });
      };
    ...
    didFail: function() {
      return function(slack) {
        return slack.notify({
          attachments: [{
            fallback: "Failed `cardboard` commit " + `${sha} to ${targetName} (${deployTarget})`,
            text: "Failed `cardboard` commit " + `${sha} to ${targetName} (${deployTarget})`,
            color: "danger"
          }]
        });
      };
